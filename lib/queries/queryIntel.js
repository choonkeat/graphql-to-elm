...
var mapInputFieldCache = {};
var maxDepth = 10;
var mapInputField = function (field, schema, depth) {
    var namedType = graphql_1.getNamedType(field.type);
    var nullableType = graphql_1.getNullableType(field.type);
    var typeName = namedType.name;
    var value = undefined;
    if (namedType instanceof graphql_1.GraphQLInputObjectType && depth < maxDepth) {
        var fields_1 = namedType.getFields();
        value = {
            kind: "object",
            typeName: typeName,
            fields: Object.keys(fields_1).map(function (key) {
                var found = mapInputFieldCache[String(fields_1[key].type)];
                if (found) {
                    return __assign(__assign({}, found), { name: fields_1[key].name });
                }
                return mapInputField(fields_1[key], schema, depth + 1);
            })
        };
    }
